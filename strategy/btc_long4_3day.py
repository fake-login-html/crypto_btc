...
class Crypto:
    ...
    def SignalBd(self, last_price, proc):
        ...
        sms = f'🛑 ВНИМАНИЕ!!! 🛑\n\n' \
              f'📚 В течении 72 часов {self.price['moneta']} упал на {round(self.price['proc'], 2)}% 📚 \n\n' \
    ...
    def CheckProc(self, last_price):
        ...
        # получаем все сигналы за крайние 3 дня
        signal = db().signal_check_3day()

        # если нет сигналов за крайние 3 дня, добавляем
        coll_orders = BitGetApi().coll_orders() # кол-во ордеров
        if not signal:
            # максимальный курс за крайние 3 дня
            max_db = db().price_check_3day()[0][0]
            proc = 100 - ((last_price / max_db) * 100)
            print('Разница от максимальной цены курса за крайние 3 дня', round(proc, 2))
