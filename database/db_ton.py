...
class DataBase:
    ...
    def __init__(self):
        ...
        # подключаемся к бд
        self.conn = psycopg2.connect(dbname='сrypto', user='satoshi', password='satoshi', host='localhost')
        # создаем курсор
    ...
    def signal_add_2_8(self, moneta):
        ...
        self.cursor.execute(f"INSERT INTO public.signal(\n" \
                       f"date, price, prognoz, moneta, use, procent, order_id)\n" \
                       f"VALUES ('{moneta['date']}', {moneta['price']}, {moneta['signal']}, '{moneta['moneta']}', false, 1.4, '{moneta['order_id']}');")
        self.cursor.close()  # закрываем курсор
        self.conn.commit()  # сохраняем изменения
        self.conn.close()  # закрываем соединение
    ...
    def price_check(self):
        ...
        # выполняем запрос
        self.cursor.execute(f"SELECT max(price) FROM price where date > '{dm}' and moneta = 'TON';")
    ...
    def signal_check(self):
        ...
        dm = (datetime.now() - timedelta(days=2)).strftime("%Y-%m-%d %H:%M:%S")
        # выполняем запрос
        self.cursor.execute(f"SELECT * FROM public.signal where date > '{dm}' and use = false and procent = {2} and moneta = 'TON' ORDER by date desc LIMIT 1;")
    ...
    def signal_check_2_8(self):
        ...
        dm = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S")
        # выполняем запрос
        self.cursor.execute(f"SELECT * FROM public.signal where date > '{dm}' and use = false and procent = '{1.4}' and moneta = 'TON' ORDER by date desc LIMIT 1;")
        signal = self.cursor.fetchall()
        self.cursor.close()  # закрываем курсор
        self.conn.close()  # закрываем соединение
        return signal
    ...
    def itog_check(self):
        # выполняем запрос
        self.cursor.execute(f"SELECT * FROM public.signal where  use = false and moneta = 'TON'")
        signal = self.cursor.fetchall()
        self.cursor.close()  # закрываем курсор
        self.conn.close()  # закрываем соединение
        return signal
